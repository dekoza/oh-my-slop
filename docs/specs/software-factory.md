...
- **One worktree per attempt**, created fresh at claim time, exactly one worker, never reused.
  ...
  attempt branch from* — and it is never the boundary of what §7.5 replays: the two values
  coincide only for a single-attempt execution, and §7.5 derives its replay set from the graph
  without consulting this one (#161).
