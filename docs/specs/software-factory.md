...
provider's fault, so §8.10 charges no budget for it) · `routes-exhausted` (#155 — every profile
§11.5's order names for this role belongs to a class §9.8's memo has locked; no worker ran, and it
is what the walk answers *instead of* minting the next attempt).
...
| implement | `routes-exhausted` | `released` (§9.9 — every routable profile is memo-locked) | — |
...
**What bounds it is that each routable profile is dispatched at most once per ticket execution**, read
back from the mints — the bound and the spend are one expression, as §8.6's budgets already are, and
there is no counter to declare or forget. An automation retry adds nothing to that spend: a pane that
died says nothing about its provider, and excluding its profile would turn every infra flake into a
silent model change.
