...
| **Attempt** | `attempt_id` = `<run_ulid>-t<ticket>-a<n>` | `n` a per-ticket-execution ordinal allocated transactionally |
...
- **One worktree per attempt**, created fresh from the pinned base commit at claim time,
  exactly one worker, never reused.
...
- **Harvest-side (builder faults):** an attempt is harvestable as `completed` only if its
  ...
  typed failure, never auto-committed — **and** its branch has **≥1 commit** ahead of the
  pinned base.
...
axis ⇒ reject.** Fowler baseline smells are judgement calls by the skill's own text and can
therefore **never** be `blocking`. This obligation lives in the **prompt template**, not inside
the package skill.
