...
carrying §10.3's exit 1, exactly as `migrate` already answers a repo-less invocation. **The
exemption is `stop` and `migrate` and nothing else** — `start` and every other verb that decides
anything from policy stay fail-closed.
