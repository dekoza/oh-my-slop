...
- **`kind: pi` with no endpoint bound** — the **provider segment of the model id**
  ...
  #209 keeps. The provider segment must match the same `[0-9A-Za-z.-]+` grammar as §9.4's
  `capacity:model:<class>:<i>` row; otherwise the profile is a **load error**, before a slot can
  be taken and stranded under a name recovery cannot read. The refusal names both migrations:
  rename the provider in pi's model catalogue, or bind the profile's endpoint so its class is
  endpoint-derived. **`endpoint-…` is reserved**: a model whose provider segment claims that
...
| 2026-09-05 | #222 applies §9.4's capacity-row grammar at the remaining class derivation boundary. An endpointless `kind: pi` profile whose provider segment cannot spell a `capacity:model:<class>:<i>` row now refuses at load, rather than loading through preflight and failing at first dispatch before the tracker claim. The loader reads the lease module's class predicate — the same segment used to build, admit, and parse the row — and names both operator migrations: rename the provider in pi's model catalogue or bind the profile's endpoint. | #222 |
