...
Ask if user wants to add or remove any patterns from the blocked list. Edit the copied script accordingly. In particular, plain `git push` is blocked by default — if the user routinely wants the agent to push to their own remotes (e.g. a self-hosted Gitea), suggest narrowing that pattern to `push --force`.
