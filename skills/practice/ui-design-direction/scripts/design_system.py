...
class DesignSystemGenerator:
    ...
    def _multi_domain_search(
        ...
        for domain, config in SEARCH_CONFIG.items():
            ...
            elif domain == "landing" and landing_priority:
                combined_query = f"{query} {landing_priority}"
    ...
    def generate(self, query: str, project_name: str = None) -> dict:
        ...
        # Step 3: Multi-domain search with category-specific priority hints
        search_results = self._multi_domain_search(
            query,
            style_priority,
            reasoning.get("pattern"),
        )
        ...
        best_landing = self._select_best_match(
            landing_results,
            [reasoning.get("pattern", "")],
        )
...
def _project_slug(project_name: str) -> str:
    """Build a stable project slug for persisted output paths."""
    return (project_name or "default").lower().replace(' ', '-')
...
def format_master_md(design_system: dict) -> str:
    ...
    lines.append("")
    lines.append(
        f"> **LOGIC:** When building a specific page, first check `design-system/{project_slug}/pages/[page-name].md`."
    )
    lines.append("> If that file exists, its rules **override** this Master file.")
    lines.append(
        f"> If not, strictly follow `design-system/{project_slug}/MASTER.md`."
    )
    lines.append("")
    lines.append("---")
    lines.append("")
...
def format_page_override_md(design_system: dict, page_name: str, page_query: str = None) -> str:
    ...
    lines.append("")
    lines.append(
        f"> ⚠️ **IMPORTANT:** Rules in this file **override** `design-system/{project_slug}/MASTER.md`."
    )
