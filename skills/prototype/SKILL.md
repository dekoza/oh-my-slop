...
2. **One command to run.** Whatever the project's existing task runner supports — `uv run`, `python <path>`, `node <path>`, etc. The user must be able to start it without thinking.
